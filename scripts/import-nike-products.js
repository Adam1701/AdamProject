...
const genderMapping = {
  ...
  'KIDS': 'kids',
  'BABY': 'kids',
  'UNISEX': 'men' // Par défaut vers men
};

// Fonction pour récupérer les produits depuis l'API
async function fetchNikeProducts(page = 1, limit = 50) {
  try {
...
// Fonction pour mapper les données de l'API vers notre schéma
function mapProductData(apiProduct) {
  const category = genderMapping[apiProduct.gender] || 'men';
  ...
  return {
    ...
    currency: 'USD', // L'API retourne des prix en USD
    variants: [{
...
async function ensureSizesExist(sizes, category) {
...
async function importProduct(apiProduct) {
  try {
    ...
    const productData = mapProductData(apiProduct);
...
async function importNikeProducts(maxPages = 5, productsPerPage = 20) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    try {
      ...
      const sneakers = products.filter(p => p.product_type === 'Sneakers');
      ...
      for (const product of sneakers) {
        try {
          ...
          totalErrors++;
          console.error(`Erreur avec le produit ${product.title}:`, error.message);
        }
      }
...
if (require.main === module) {
  importNikeProducts(3, 20) // 3 pages, 20 produits par page
    .then(() => {
...
module.exports = { importNikeProducts, fetchNikeProducts, importProduct };
